...
class Feedback360 {
    ...
    boolean readPosition() {
        ...
        double dc;
        while (true) {
            tHigh = pulseIn(this->feedbackPin, HIGH, 10000);
            tLow = pulseIn(this->feedbackPin, LOW, 10000);
            tCycle = tHigh + tLow;
            if (1000 < tCycle && tCycle < 1200) break; // nominal time is 1100us
            if (tCycle < 10) return false; // pulseIn timed out, abort
        }
        ...
        this->position = this->turns + this->theta;
        double dt = (millis() - this->t) / 1000.0;
        this->t = millis();
        this->speed = (this->position - this->pPos) / dt;
        return true;
    }
